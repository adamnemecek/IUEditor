...
- (IUCSSCode*)cssCodeForIU:(IUBox*)iu target:(IUTarget)target viewPort:(NSInteger)viewPort option:(NSDictionary *)option {
    ...
    if (target == IUTargetEditor) {
        ...
        IUPositionStorage *storage = (IUPositionStorage *)[[iu positionManager] cascadingStorageForViewPort:viewPort];
        [self updateCSSPositionCode:code IU:iu storage:storage option:option];
        
        IUStyleStorage *styleStorage = (IUStyleStorage *)[[iu defaultStyleManager] cascadingStorageForViewPort:viewPort];
